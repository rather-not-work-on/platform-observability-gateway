...
ALLOWED_ACTION = {
    "none",
    "monitor_replay_latency",
    "fix_event_payload_contract",
    "investigate_replay_sequence",
}
...
def validate_map_doc(map_doc: dict, taxonomy_doc: dict):
    ...
    if map_doc.get("taxonomy_source") != "config/observability-reason-taxonomy.json":
        errors.append("taxonomy_source must be config/observability-reason-taxonomy.json")
...
def main():
    parser = argparse.ArgumentParser(description="Validate delay/replay reason taxonomy map")
    parser.add_argument("--map", default="config/delay-replay-reason-taxonomy-map.json")
    parser.add_argument("--taxonomy", default="config/observability-reason-taxonomy.json")
    parser.add_argument(
        "--output", default="runtime-artifacts/validation/delay-replay-reason-taxonomy-map-report.json"
    )
